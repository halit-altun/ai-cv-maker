        ...
        >
          ...
          {(() => {
            ...
            if (!recipients.length) {
              return (
                <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
                  Alıcı seçince cold mail önizlemesi burada görünür. Tek {GENERIC_INBOX_LABEL} ise yalnızca
            ...
            return (
              <Stack spacing={2} sx={{ mb: 2 }}>
                ...
                {showInfoContact && (
                  <Box>
                    <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 0.5 }}>
                      {GENERIC_INBOX_LABEL} özel cold mail
                    </Typography>
                    <Typography variant="caption" color="text.secondary" display="block" sx={{ mb: 1 }}>
                      {onlyInfo
                        ? `Yalnızca girilen ana adres ${GENERIC_INBOX_LOCALS_SLASH} olduğu için standart sürüm üretilmez / gösterilmez; bu metin kullanılır.`
                        : `${GENERIC_INBOX_LABEL} adreslerine İK yönlendirme girişli bu sürüm gider; diğer alıcılar standart metni alır.`}
                    </Typography>
                    <TextField
                      fullWidth
                      label={`${GENERIC_INBOX_LOCALS_SLASH} mail gövdesi`}
          ...
          >
            ...
            <DialogContent dividers>
              ...
              <Alert severity="info" sx={{ my: 1, py: 0.5 }}>
                {onlyInfoOrContactEmails(props.selectedOutreachRecipients)
                  ? `Seçili alıcılar ${GENERIC_INBOX_LABEL} — yalnızca yönlendirmeli cold mail gönderilir.`
                  : anyInfoOrContactEmail(props.selectedOutreachRecipients)
                    ? `Karışık liste: ${GENERIC_INBOX_LOCALS_SLASH} adreslerine özel sürüm, diğerlerine standart cold mail.`
