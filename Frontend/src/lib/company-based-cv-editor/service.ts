import { 
  ...
  CVAnalysisRequest, 
  CVAnalysisResponse,
  CompanyOptimizationBundleRequest,
  CompanyOptimizationBundleResult,
  GeminiAPIResponse,
  CompanyBasedCVData
} from './types';
...
import {
  ...
  countWords,
  formatBudgetGuidance,
} from './wordLengthBudget';
import {
...
export class CompanyBasedCVService {
  ...
  // Şirket bilgilerini analiz et — sayfalar paralel fetch, AI tek istek
  static async analyzeCompany(companyUrls: CompanyLink[]): Promise<CompanyInfo> {
    console.log('=== COMPANY ANALYSIS STARTED (single AI) ===');
    console.log('Number of links to analyze:', companyUrls.length);

    const fetchedPages = await Promise.all(
      companyUrls.map(async (link, index) => {
        const pageTypeLabel =
          link.pageType === 'other'
            ? String(link.pageTypeOther || '').trim() || 'Diğer'
            : link.pageType || link.description || 'Belirtilmedi';
        ...
        // Prompt boyutu için sayfa metnini sınırla
        const clipped =
          pageText.length > 12000
            ? `${pageText.slice(0, 12000)}\n…[truncated]`
            : pageText;

        console.log(`Link ${index + 1} fetch: ${clipped ? `${clipped.length} chars` : 'empty'}`);
...
--- PAGE ${i + 1} ---
URL: ${p.link.url}
Page type: ${p.pageTypeLabel}
Context: ${p.link.description || p.pageTypeLabel}
FETCHED TEXT:
${p.pageText || '(Sayfa metni alınamadı — URL/tipine göre abartmadan çıkarım yap, uydurma bilgi üretme.)'}
`
      )
      .join('\n');

    const prompt = `
Aşağıdaki şirket web sayfalarını TEK seferde analiz et ve birleşik şirket profili üret.
...
JSON formatında cevap ver:
{
  ...
  "extractedKeywords": ["KW1", "KW2", "KW3"],
  "detectedLanguage": "turkish",
  "analyzedLinks": [
    { "url": "...", "description": "...", "pageType": "careers" }
  ]
}

Kurallar:
1. Tüm sayfaları birleştir; çelişkide en net/güncel bilgiyi kullan.
...
`;
    ...
    const finalResult = this.parseJSONResponse(response);

    if (!finalResult.detectedLanguage || finalResult.detectedLanguage === 'other') {
      const sample = fetchedPages.map((p) => p.pageText).join(' ').slice(0, 4000);
      const tr = (sample.match(/[çğıöşüÇĞİÖŞÜ]/g) || []).length;
      const en = (sample.match(/\b(the|and|with|for|our|we)\b/gi) || []).length;
      if (tr > en) finalResult.detectedLanguage = 'turkish';
      else if (en > tr) finalResult.detectedLanguage = 'english';
    }

    const fromFinal = Array.isArray(finalResult.extractedKeywords)
      ? finalResult.extractedKeywords.map((k: unknown) => String(k ?? '').trim()).filter(Boolean)
      : typeof finalResult.extractedKeywords === 'string'
        ? String(finalResult.extractedKeywords)
            .split(',')
            .map((k) => k.trim())
            .filter(Boolean)
        : [];
    finalResult.extractedKeywords = fromFinal.slice(0, 10);

    if (!Array.isArray(finalResult.analyzedLinks) || finalResult.analyzedLinks.length === 0) {
      finalResult.analyzedLinks = companyUrls.map((l) => ({
        url: l.url,
  ...
  /**
   * Optimizasyon adımı: CV parse + uyarlama + (opsiyonel) cover/LinkedIn/cold mail
   * tek Gemini isteğinde.
   */
  static async runFullOptimizationBundle(
    request: CompanyOptimizationBundleRequest
  ): Promise<CompanyOptimizationBundleResult> {
    ...
    const needCompanyExtract = adaptSource === 'company' && !request.companyInfo && hasCompanyPages;
...
`;
    ...
    for (let attempt = 0; attempt <= GEMINI_MAX_RETRIES; attempt++) {
      try {
        const raw = await this.callGeminiAPI(prompt, { jsonMode: true });
        const parsed = this.parseJSONResponse(raw, { useCompanyFallback: false }) as Record<
          string,
          unknown
        >;

        const parsedCV = this.normalizeParsedCVData(parsed.parsedCV || {}, request.cvText);
        const analysis = this.normalizeCVAnalysisResponse(
        ...
        let linkedinMessage = wantLinkedIn ? String(parsed.linkedinMessage || '').trim() : '';

        if (coverLetter) {
          coverLetter = this.normalizeOutreachLetterFormatting(
            coverLetter.replace(/\[company\]/gi, '')
          );
          if (!recipientCompany && request.companyInfo?.name) {
            const escaped = request.companyInfo.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            coverLetter = this.normalizeOutreachLetterFormatting(
              coverLetter.replace(new RegExp(escaped, 'gi'), '')
            );
          }
          const signature = this.buildOutreachSignatureBlock(parsedCV.personalInfo);
          coverLetter = `${coverLetter}\n\n${signature}`.trim();
        }

        if (linkedinMessage) {
          linkedinMessage = this.normalizeOutreachLetterFormatting(
        ...
        let coldEmail: { subject: string; body: string } | null = null;
        if (wantCold) {
          const coldRaw = (parsed.coldEmail || {}) as { subject?: string; body?: string };
          let subject = String(coldRaw.subject || '').trim();
          let body = String(coldRaw.body || '').trim();
          const fullName = `${parsedCV.personalInfo?.firstName || ''} ${parsedCV.personalInfo?.lastName || ''}`.trim();
          const title =
          ...
          if (body) {
            const linkedin =
              (request.outreachLinkedinUrl || '').trim() ||
              parsedCV.personalInfo?.linkedin ||
              '';
            const portfolio =
              (request.outreachPortfolioUrl || '').trim() ||
              parsedCV.personalInfo?.portfolio ||
              '';
            const website = (request.outreachWebsiteUrl || '').trim();
        ...
        let resolvedCompanyInfo: CompanyInfo | undefined = request.companyInfo;
        if (needCompanyExtract && parsed.companyInfo && typeof parsed.companyInfo === 'object') {
          const raw = parsed.companyInfo as Record<string, unknown>;
          ...
          resolvedCompanyInfo = {
            ...
            website: String(raw.website || request.companyPages?.[0]?.url || '').trim(),
            ...
            analyzedLinks: Array.isArray(raw.analyzedLinks)
              ? (raw.analyzedLinks as CompanyInfo['analyzedLinks'])
              : (request.companyPages || []).map((p) => ({
                  url: p.url,
                  description: p.description || p.pageType || '',
                  pageType: (p.pageType || undefined) as CompanyLink['pageType'],
                })),
          };
        }
        ...
        console.warn(
          `[runFullOptimizationBundle] parse fail — retry ${attempt + 1}/${GEMINI_MAX_RETRIES + 1}`
        );
  ...
  static async analyzeAndAdaptCV(request: CVAnalysisRequest): Promise<CVAnalysisResponse> {
    ...
    const manualMustNotMention = Array.isArray(request.manualMustNotMentionTopics)
      ...
      : [];
    const keywordTargetAbout = request.keywordTargetSections?.about === true;
    const keywordTargetExperience = request.keywordTargetSections?.workExperience === true;
    const companyKeywords = Array.isArray(request.companyInfo?.extractedKeywords)
      ? request.companyInfo.extractedKeywords.filter(Boolean)
      : [];
    ...
    const aboutSourceText = String(request.currentAbout || '').trim();
    const aboutBudget = computeWordLengthBudget(countWords(aboutSourceText));

    const workItems = Array.isArray(request.currentWorkExperience)
      ? request.currentWorkExperience
      : [];

    const bulletBudgetLines: string[] = [];
    workItems.forEach((exp, expIndex) => {
      const bullets = Array.isArray(exp.bulletPoints) ? exp.bulletPoints : [];
      const header = [exp.position, exp.company].filter(Boolean).join(' @ ') || `Experience ${expIndex + 1}`;
      bulletBudgetLines.push(`  Experience ${expIndex + 1} (${header}) — keep EXACTLY ${bullets.length} bullet(s); DO NOT add new bullets:`);
      bullets.forEach((bp, bpIndex) => {
        const budget = computeWordLengthBudget(countWords(bp));
        bulletBudgetLines.push(
          formatBudgetGuidance(budget, `Bullet ${expIndex + 1}.${bpIndex + 1}`).replace(/^- /, '    - ')
        );
      });
    });

    const lengthBudgetRules = `
    LENGTH BUDGET RULES (CRITICAL — KEYWORD ADAPTATION MUST RESPECT WORD COUNTS):
    When weaving target keywords, expand ONLY within the budgets below. Do not pad for length.

    ABOUT SECTION:
${formatBudgetGuidance(aboutBudget, 'updatedAbout')}
    - Prefer weaving KWs inside existing sentences; +10–15 words is ideal when KWs are added.
    - Never roughly double the about length.

    WORK EXPERIENCE BULLETS (per bullet — NOT total experience word count):
${bulletBudgetLines.length ? bulletBudgetLines.join('\n') : '  - No structured bullets provided; keep each rewritten bullet close to its original length (+10–15 preferred, +25 hard max).'}
    - Adapt ONLY existing bullets. NEVER invent/add extra bullet points.
    - Keep the same number of bullets per job as in the original CV.
    - Never rewrite a short bullet into a long paragraph.
    `;

    const keywordIntegrationRules = `
    KEYWORD INTEGRATION RULES (CRITICAL — FOLLOW EXACTLY):
    Source: ${adaptationSource === 'text' ? 'JOB DESCRIPTION' : 'COMPANY PAGES'}
    Company-extracted keywords (hint): ${companyKeywords.length ? companyKeywords.join(', ') : 'none'}
    Allowed target sections for keyword weaving:
      - about: ${keywordTargetAbout ? 'YES' : 'NO'}
      - workExperience: ${keywordTargetExperience ? 'YES' : 'NO'}
    NEVER put forced keywords into skills / languages / education sections for this purpose.

    Step A — Build a candidate pool of up to 10 important keywords (KW) from the target source:
      - Prefer hard skills, frameworks, tools, domain terms (e.g. PHP, React, microservices, B2B SaaS).
      - Ignore soft fluff unless truly distinctive.
      - Return them as candidateKeywords (length ≤ 10).

    Step B — FULL CV SCAN (eliminate already present):
      - For each candidate, check if it already appears ANYWHERE in the CV text (about, experience, skills, education, languages, titles…).
      - If yes → DO NOT weave it again. Mark integratedIn="already_present" with note "CV'de zaten geçiyor".
      - Example: 10 candidates, 3 already in CV → those 3 are eliminated from weaving.

    Step C — Select at most 5 weave KWs from the REMAINING candidates:
      - Rank by importance to the target + natural applicability to THIS CV (without inventing facts).
      - detectedKeywords = those ≤5 weave targets only (not the already_present ones).

    Step D — Weave ONLY the ≤5 weave KWs into the PRIMARY allowed section:
      PRIMARY:
        ${
          keywordTargetAbout && !keywordTargetExperience
            ? 'about ONLY'
            : keywordTargetExperience && !keywordTargetAbout
              ? 'experience ONLY'
              : keywordTargetAbout && keywordTargetExperience
                ? 'about FIRST (primary). Prefer about; use experience only if about cannot fit a KW naturally.'
                : 'NONE — do not force-weave'
        }
      HARD RULE: After a KW is placed in one section, do NOT also insert it into the other.
      integratedIn="both" ONLY if the ORIGINAL text already had that KW in both sections.
      If a KW cannot fit safely → integratedIn="none" (do not force).

    Step E — PRESERVE ORIGINAL DETAIL (CRITICAL):
      - Adaptation = ADD KW into existing text when needed; NOT a full rewrite/summary.
      - Bullets that do not receive a KW must be copied EXACTLY from the original.
      - Bullets that receive a KW must keep ALL original details/tech/metrics; may grow +10–15 words (max +25).
      - NEVER shorten, compress, or drop details from original bullets/about.
      - NEVER invent metrics; keep existing numbers.

    Step F — Adaptation style (DO NOT FAKE EXPERTISE):
      - Weave KWs are by definition NOT already in the CV — use adaptive phrasing, NOT false ownership.
      Short examples (Turkish output language when cv is Turkish):
        * Missing PHP but has React: "React ile geliştirdiğim e-ticaret akışlarında PHP tabanlı servislerle entegrasyon deneyimi kazandım / üzerinde çalışıyorum."
        * Missing Kubernetes: "Konteynerleştirme ve orkestrasyon (Kubernetes) konularında aktif öğrenme ve proje denemeleri yapıyorum."
      If weaving would look ridiculous → SKIP and report (do not force).
      If weaving would force dropping original detail → SKIP the KW (never shorten to make room).

    Step G — Report:
      keywordIntegrationReport must include:
        - already_present KWs
        - the ≤5 weave KWs (about|experience|none)
        - optional leftover candidates with integratedIn="none" ("üst 5'e giremedi")
      Shape: { "keyword": "...", "integratedIn": "about"|"experience"|"both"|"none"|"already_present", "note": "..." }
    `;
    ...
    Please respond in the following JSON format:
    {
      ...
      "matchScore": 85,
      "detectedKeywords": ["KW1", "KW2"],
      "keywordIntegrationReport": [
        { "keyword": "PHP", "integratedIn": "about", "note": "Deneyimde zorlandı; hakkımda öğrenme tonuyla geçirildi." }
      ],
      "positiveMatches": [
        {
    ...
    - keywordIntegrationReport and detectedKeywords are REQUIRED.
    ...
    ${isEnglish ? 
      ...
      1. ABOUT SECTION RULES:
         ...
         - Obey LENGTH BUDGET RULES for updatedAbout (current word count based safe/max ranges).
      ...
      2. WORK EXPERIENCE RULES:
         ...
         - ONLY REWRITE EXISTING BULLET POINT CONTENT — DO NOT ADD NEW BULLET POINTS
         - Keep the SAME bullet count per job as the original CV
         ...
         - Use numbers ONLY if already present in the original bullet (do not invent metrics)
         ...
         - Obey PER-BULLET LENGTH BUDGET RULES (+10–15 ideal, +25 hard max; never roughly double)
      ...
      1. HAKKIMDA BÖLÜMÜ İÇİN ÖZEL KURALLAR:
         ...
         - LENGTH BUDGET RULES'a uy: updatedAbout mevcut kelime sayısına göre güvenli/maks aralıkta kalsın.
      ...
      2. İŞ DENEYİMİ İÇİN ÖZEL KURALLAR:
         ...
         - SADECE MEVCUT BULLET POINT'LERİN İÇERİĞİNİ YENİDEN YAZ — YENİ BULLET EKLEME
         - Her iş deneyiminde orijinaldeki bullet SAYISINI AYNEN koru
         ...
         - Rakamları yalnızca orijinal bullet'ta varsa koru; yeni metrik uydurma
         - Somut ve net ol: "Başarılı oldum" değil → "Süreyi %15 kısalttım"
         - Hedef şirketin değerlerine uygun ama gerçek deneyimi koru
         - HER BULLET İÇİN LENGTH BUDGET RULES geçerli (+10–15 ideal, +25 sert üst; yaklaşık 2 katına çıkarma yasak)
  ...
  private static normalizeCVAnalysisResponse(
    ...
    options?: {
      cvText?: string;
      keywordTargetSections?: { about?: boolean; workExperience?: boolean };
    }
  ): CVAnalysisResponse {
    ...
    const normalizeReport = (value: unknown): CVAnalysisResponse['keywordIntegrationReport'] => {
      ...
      return value
        .map((item) => {
          ...
          const rawIn = String(row.integratedIn ?? '')
            .trim()
            .toLowerCase();
          let integratedIn: NonNullable<
            CVAnalysisResponse['keywordIntegrationReport']
          >[number]['integratedIn'] = 'none';
          if (
            rawIn === 'about' ||
            rawIn === 'experience' ||
            rawIn === 'both' ||
            ...
            integratedIn = rawIn;
          } else if (rawIn === 'workexperience' || rawIn === 'work_experience' || rawIn === 'exp') {
  ...
  static async generateCompanyCoverLetter(params: {
    ...
    const targetInfoBlock =
      source === 'text'
        ? `Job Description Text:\n${jobDescriptionText || 'Job description text is missing.'}\n\nIMPORTANT: Extract important keywords from the JD and weave them naturally into the cover letter body (without fake claims).`
        : `Target Company Information:\n${
            companyInfo
              ? `- Company Name: ${companyInfo.name}\n- Industry: ${companyInfo.industry}\n- Description: ${companyInfo.description}\n- Values: ${(companyInfo.values || []).join(', ')}\n- Requirements: ${(companyInfo.requirements || []).join(', ')}\n- Culture: ${companyInfo.culture || ''}\n- Keywords from pages: ${(companyInfo.extractedKeywords || []).join(', ') || 'N/A'}`
              : '- Company information is missing.'
          }\n\nIMPORTANT: Weave page keywords into the cover letter naturally; if unsupported by CV, use learning/interest tone — never fake mastery.`;
  ...
  /**
   * İK / işe alım için cold mail (soğuk e-posta) üretir.
   * Cover letter'dan ayrı: max ~150 kelime, doğrudan CTA, JSON { subject, body }.
   */
  static async generateCompanyColdEmail(params: {
    source: 'company' | 'text';
    companyInfo?: CompanyInfo;
    jobDescriptionText?: string;
    personalInfo?: Partial<CompanyBasedCVData['personalInfo']>;
    ...
    const isEnglish = cvLanguage === 'english';
    const fullName = `${(personalInfo?.firstName || '').trim()} ${(personalInfo?.lastName || '').trim()}`.trim();
    const title = (personalInfo?.title || targetPosition || 'Full Stack Developer').trim();
    const company =
    ...
    const highlights = Array.isArray(candidateHighlights)
      ? candidateHighlights.slice(0, 5).join('\n- ')
      : '';
    const mustMention = Array.isArray(manualMustMentionTopics)
      ? manualMustMentionTopics.filter(Boolean).join(', ')
      : '';
    const linkedin =
      (linkedinUrl || '').trim() || (personalInfo?.linkedin || '').trim() || '';
    const portfolio =
      (portfolioUrl || '').trim() || (personalInfo?.portfolio || '').trim() || '';
    const website = (websiteUrl || '').trim();
    const phone =
      (phoneOverride || '').trim() || (personalInfo?.phone || '').trim() || '';
    const email = (personalInfo?.email || '').trim();
    const city = [personalInfo?.city, personalInfo?.country].filter(Boolean).join(', ');
    const keywords =
      source === 'company' && Array.isArray(companyInfo?.extractedKeywords)
        ? companyInfo.extractedKeywords.join(', ')
        : '';

    const targetBlock =
      source === 'text'
        ? `Job / target text:\n${jobDescriptionText || 'N/A'}`
        : `Company pages summary:\n${
            companyInfo
              ? JSON.stringify(
                  {
                    ...
                    keywords: companyInfo.extractedKeywords,
                  },
                  null,
                  2
                )
    ...
    const experienceRule =
      candidateExperienceYears != null
        ? `Mention experience years ONLY as exactly ${candidateExperienceYears} if needed; never invent other tenure.`
        : `Do NOT mention years of experience.`;

    const optionalLinksBlock = [
...
3. PERSONALIZATION (RESEARCHED VIBE) + HARD GROUNDING:
   ...
   - If target text has little/no clear work-area signal: skip domain invention — open with name + role interest in [Company] only. Sparse truth > fake research.
   MATCH PRIORITY (CRITICAL):
   - First prioritize REAL OVERLAP: skills/tech/experience present in BOTH target text AND candidate CV/skills/highlights.
   - NEVER claim you "have" / "master" a domain technology that is NOT in the CV (example: telecom company + missing stack X → forbidden: "telekomünikasyon için X teknolojisine sahibim").
   - If company domain is clear but CV lacks that tech: use interest/willingness only ("bu alana katkı vermek istiyorum", "ilgi duyuyorum") — not ownership.
   - Bridge only with CV-supported skills (e.g. React/.NET) toward the grounded domain; do not invent telecom-specific tools.
4. STRUCTURE:
...
- About: ${about || 'N/A'}
- Skills (CV): ${skills}
- Highlights (use only these for numbers):
- ${highlights || 'N/A'}
- Must-mention topics (if any): ${mustMention || 'none'}
- Recipient name (optional): ${recipient || 'none'}
- Company: ${company}
- Tech keywords from target (hint): ${keywords || 'none'}

${targetBlock}

OPTIONAL SIGNATURE LINKS (include only if non-empty; otherwise omit that line):
${optionalLinksBlock || '(none provided)'}
...
`;
    ...
    const response = await this.callGeminiAPI(prompt, { jsonMode: true });
    const parsed = this.parseJSONResponse(response, { useCompanyFallback: false }) as {
      subject?: string;
      body?: string;
    };
  ...
  static async generateCompanyLinkedInMessage(params: {
    ...
    const targetInfoBlock =
      source === 'text'
        ? `Job Description Text:\n${jobDescriptionText || 'Job description text is missing.'}\n\nIMPORTANT: Weave important JD keywords naturally into the LinkedIn message (no fake claims).`
        : `Target Company Information:\n${
            companyInfo
              ? `- Company Name: ${companyInfo.name}\n- Industry: ${companyInfo.industry}\n- Description: ${companyInfo.description}\n- Values: ${(companyInfo.values || []).join(', ')}\n- Requirements: ${(companyInfo.requirements || []).join(', ')}\n- Culture: ${companyInfo.culture || ''}\n- Keywords from pages: ${(companyInfo.extractedKeywords || []).join(', ') || 'N/A'}`
              : '- Company information is missing.'
          }\n\nIMPORTANT: Weave page keywords into the LinkedIn message naturally; use learning/interest tone if CV lacks them.`;
  ...
  /**
   * Gemini çağrısı Backend proxy üzerinden (JWT + client_id).
   * API key tarayıcıda tutulmaz; yalnızca round-robin key indeksi (X/Y/Z) gönderilir.
   * Sekmeler arası indeks localStorage + Web Locks ile senkronize edilir.
   */
  private static async callGeminiAPI(
    ...
    for (let attempt = 0; attempt <= GEMINI_MAX_RETRIES; attempt++) {
      const logPrefix = `[Gemini proxy deneme ${attempt + 1}/${GEMINI_MAX_RETRIES + 1}]`;
      const preferredKeyIndex = await claimGeminiKeyIndex(getGeminiKeyCount());

      try {
        const response = await authFetch('/api/ai/gemini', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            prompt,
        ...
        if (!response.ok || data.ok === false) {
          ...
          // Veritabanı kesintisinde beklemek fayda etmez, hemen bildir
          if (data.code === 'DATABASE_UNAVAILABLE') {
            throw lastError;
          }
          if (this.isRetriableGeminiHttpStatus(response.status) && attempt < GEMINI_MAX_RETRIES) {
            console.warn(
              `${logPrefix} ${response.status} (claimed key index ${preferredKeyIndex}) — ${GEMINI_RETRY_DELAY_MS / 1000} sn sonra yeni key indeksi ile tekrar`
            );
            await new Promise((resolve) => setTimeout(resolve, GEMINI_RETRY_DELAY_MS));
            continue;
          }
        ...
        if (text) {
          if (typeof data.keyCount === 'number' && data.keyCount >= 1) {
            setGeminiKeyCount(data.keyCount);
          }
          if (typeof data.keyIndexUsed === 'number' && Number.isFinite(data.keyIndexUsed)) {
            syncGeminiKeyIndexAfterUse(data.keyIndexUsed, data.keyCount ?? getGeminiKeyCount());
          }
          if (data.rateLimitInfo?.waited) {
            console.log(
              `${logPrefix} rate limit koruması — ${(data.rateLimitInfo.waitedMs / 1000).toFixed(1)}s beklendi (çoklu sekme/paralel istek koruması)`
            );
          }
          console.log(
            `${logPrefix} başarılı — model=${data.model ?? COMPANY_BASED_GEMINI_MODEL} key=${data.keyUsed ?? '?'} index=${data.keyIndexUsed ?? preferredKeyIndex}`
          );
          await new Promise((resolve) => setTimeout(resolve, GEMINI_POST_SUCCESS_DELAY_MS));
          return text;
        }

        lastError = new Error('Gemini proxy boş yanıt döndü');
        ...
        if (isNetwork && attempt < GEMINI_MAX_RETRIES) {
          console.warn(`${logPrefix} ağ hatası — yeni key indeksi ile tekrar:`, error);
          await new Promise((resolve) => setTimeout(resolve, GEMINI_RETRY_DELAY_MS));
          continue;
        }
  ...
  /** JSON string değerleri içindeki kaçışsız satır sonlarını / kontrol karakterlerini düzeltir. */
  private static escapeControlCharsInJsonStrings(jsonStr: string): string {
    ...
    for (let i = 0; i < jsonStr.length; i++) {
      ...
      // Diğer kontrol karakterleri JSON'da geçersiz
      if (code < 0x20) {
  ...
  /**
   * String içindeki kaçışsız `"` karakterlerini düzeltir.
   * Gemini bazen "Next.js" gibi değerlerde tırnak kaçırmadan üretir.
   */
  private static escapeUnescapedQuotesInJsonStrings(jsonStr: string): string {
    let result = '';
    let inString = false;
    let escaped = false;

    const isStructuralAfterQuote = (from: number): boolean => {
      let j = from;
      while (j < jsonStr.length && /\s/.test(jsonStr[j])) j++;
      if (j >= jsonStr.length) return true;
  ...
  /**
   * Token limitiyle yarıda kesilmiş JSON'u kapatmayı dener.
   */
  private static repairTruncatedJson(jsonStr: string): string | null {
    const start = jsonStr.indexOf('{');
    if (start === -1) return null;

    let s = jsonStr.slice(start);
    let inString = false;
    let escaped = false;
    const stack: Array<'{' | '['> = [];
    ...
    if (inString) {
      // Sondaki yarım escape varsa temizle
    ...
    // Açık yapıları ters sırada kapat
  ...
  private static tryParseJsonCandidates(response: string): unknown | null {
    ...
    const pipeline = (input: string): string[] => {
      const sanitized = this.sanitizeJsonString(input);
      const withControls = this.escapeControlCharsInJsonStrings(sanitized);
      const withQuotes = this.escapeUnescapedQuotesInJsonStrings(withControls);
      const repaired =
        this.repairTruncatedJson(withQuotes) ??
        this.repairTruncatedJson(withControls) ??
        this.repairTruncatedJson(sanitized);

      return [input, sanitized, withControls, withQuotes, repaired].filter(
        (v): v is string => typeof v === 'string' && v.includes('{')
      );
    };

    const candidates = Array.from(
      new Set([...pipeline(stripped), ...pipeline(extracted)])
    );
  ...
  private static parseJSONResponse(
    ...
    console.error('JSON parsing failed. Tail:', response.slice(-300));
    ...
    try {
      ...
      for (let attempt = 0; attempt <= GEMINI_MAX_RETRIES; attempt++) {
        try {
          ...
          // CV parse'ta şirket fallback'i kullanma — yanlış şema dönmesin
          const parsedData = this.parseJSONResponse(response, { useCompanyFallback: false });
          ...
          console.warn(
            `[parseCVDataWithAI] JSON parse başarısız — deneme ${attempt + 1}/${GEMINI_MAX_RETRIES + 1}, yeniden çağrılıyor...`
          );
          await new Promise((resolve) => setTimeout(resolve, GEMINI_RETRY_DELAY_MS));
        }
      }
