...
async function deleteProjectCompanyHandler(req, res, next) {
  try {
    const domain = req.body?.domain || req.query?.domain;
    const result = await deleteProjectCompany(req.clientId, req.params.id, domain);
    return res.json({ ok: true, ...result });
  } catch (error) {
    if (sendError(res, error)) return undefined;
    return next(error);
...
async function deleteProjectLogHandler(req, res, next) {
  try {
    const result = await deleteProjectLog(req.clientId, req.params.id, req.params.logId);
    return res.json({ ok: true, ...result });
  } catch (error) {
    if (sendError(res, error)) return undefined;
    return next(error);
